...
public class ValueDeckProvider extends DeckProvider {

    private Value value;

    private ValueDeckProvider() {
    }

    public ValueDeckProvider(Value value) {
        this.value = value;
    }
    ...
    @SneakyThrows
    public Deck provide() {
        String cardsToProvade = Stream.of(Suit.values()).map((suit) -> {
