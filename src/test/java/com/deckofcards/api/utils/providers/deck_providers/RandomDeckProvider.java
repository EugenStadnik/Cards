...
public class RandomDeckProvider extends DeckProvider {

    private int count;

    private RandomDeckProvider() {
    }

    public RandomDeckProvider(int count) {
        this.count = count;
    }
    ...
    @SneakyThrows
    public Deck provide() {
        String cardsToProvade = Stream.generate(() -> "").limit(count).map((s) -> {
            return Value.getRandom().getShortName() + Suit.getRandom().getShortName();
